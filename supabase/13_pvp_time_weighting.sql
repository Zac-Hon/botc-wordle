...
begin
  ...
  if v_max is not null and v_max > 13 then
    raise exception 'Refusing to run 13_pvp_time_weighting.sql: migration % is already applied. Apply the numbered files in order, or not at all.', v_max;
  end if;
end
$guard$;
...
insert into public.schema_version (n) values (13) on conflict (n) do nothing;
